...
class PreloaderFailSafeTests(TestCase):
    ...
    def test_main_js_has_preloader_fail_safe(self):
        ...
        self.assertIn("function hideAfterLoad", main_js)
        self.assertIn('preloader.classList.add("is-hidden")', main_js)
        self.assertNotIn("DOMContentLoaded", main_js)
        self.assertIn('window.addEventListener("load", hideAfterLoad', main_js)
        self.assertIn("window.setTimeout(hidePreloader, PRELOADER_AFTER_LOAD_DELAY_MS)", main_js)
        self.assertIn("PRELOADER_AFTER_LOAD_DELAY_MS = 200", main_js)
        self.assertIn("fallbackTimer = window.setTimeout(hidePreloader, PRELOADER_TIMEOUT_MS)", main_js)
    ...
    def test_main_min_js_has_preloader_fail_safe(self):
        ...
        self.assertIn("hideAfterLoad", main_min_js)
        self.assertIn("is-hidden", main_min_js)
        self.assertNotIn("DOMContentLoaded", main_min_js)
        self.assertIn("addEventListener(\"load\",hideAfterLoad", main_min_js)
        self.assertIn("PRELOADER_AFTER_LOAD_DELAY_MS=200", main_min_js)
