...
def resolve_state(*, has_intent: bool, has_name: bool, has_company: bool, has_phone: bool, has_email: bool, requires_email: bool = False, locked: bool = False) -> LeadStateSnapshot:
    if locked:
        return LeadStateSnapshot(state=LeadState.CLOSED, next_field="", is_terminal=True)
    if has_name and (has_phone or has_email):
        if requires_email and not has_email:
    ...
    if not has_intent and not has_name and not has_company and not has_phone and not has_email:
    ...
    if not has_email and requires_email:
        return LeadStateSnapshot(state=LeadState.COLLECT_EMAIL, next_field="email", is_terminal=False)
